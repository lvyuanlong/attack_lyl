...
print('噪声长度: %02d, lr: %d, 迭代次数: % d, 目标攻击: % s' % (args.noise_len, args.lr, args.max_steps, args.target))
...
def Attack(speaker_model, enroll_speaker_feat, train_speaker, speaker, target, args):
    ...
    # 生成时延序列
    delay_list = [i for i in range(0, adv_len, 1600)]  # 160为0.01s 1600为0.1s
    ...
    for i in range(max_steps):
        ...
        # 选取时延
        delay = random.randint(0, len(delay_list)-1)
        delay = delay_list[delay]
        ...
        for b in range(batch_num):
            ...
            # 将扰动重复的与最长的语音一样长并长出一个噪声长度
            adv_noise_tmp = adv_noise.repeat((max_len + adv_len) // adv_len + 1)[:max_len + adv_len]

            # 获得加上时延的扰动
            adv_noise_tmp = adv_noise_tmp[delay: max_len+delay]
            ...
            if not args.rir:
                ...
                rir_tensor = torch.FloatTensor(
                    loadWAV(os.path.join(args.attack_rir_path, random.choice(rir_wavs_train)))).cuda()
            ...
            if args.target:
                ...
                loss1 = torch.sum(torch.clamp(dist_batch + args.margine, min=0.))
    ...
    for i in range(max_steps):
        ...
        for b in range(batch_num):
            ...
            # # loss2
            # spec_stft = torch.stft(adv_noise / 2 ** 15, n_fft=320, hop_length=160, win_length=320,
            #                        window=torch.hamming_window(320).cuda(), return_complex=True)
            # spec_stft = torch.view_as_real(spec_stft)
            # loss2 = torch.mean(torch.sqrt(torch.square(spec_stft[:, :, 0]) + torch.square(spec_stft[:, :, 1])))

            # l2
            loss2 = torch.square(torch.norm(adv_noise/2**15, p=2))
        ...
        grad_new = momentum * grad_pre + alpha * grad1 / (
                    np.linalg.norm(grad1, 1) + 1e-12) + beta * grad2 / np.linalg.norm(grad2, 1)
...
def Test(speaker_model, enroll_speaker_feat, test_speaker, speaker, target, adv_noise, args):
    ...
    # 生成时延序列
    delay_list = [i for i in range(0, adv_len, 800)]  # 160为0.01s 1600为0.1s
    ...
    for wav in wavs:
        ...
        adv_noise_tmp = np.tile(adv_noise, wav_data.shape[1] // adv_noise.shape[0] + 2)[0: (wav_data.shape[1] + len(adv_noise))]
        for delay in delay_list:
            # # pad the adversarial perturbation to the same length with one test audio
            # adv_noise_tmp = np.tile(adv_noise, wav_data.shape[1] // adv_noise.shape[0] + 2)[0:wav_data.shape[1]+len(adv_noise)]

            noise_tmp = adv_noise_tmp[delay: delay + wav_data.shape[1]]

            # if doing rir simulation, we have to reverberate the audio and perturbation
            if not args.rir:
                wav_data = wav_data + np.expand_dims(noise_tmp, axis=0)
            else:
                rir_np = loadWAV(os.path.join(args.attack_rir_path, random.choice(rir_wavs_test)))
                rir_np = rir_np / np.linalg.norm(rir_np)
                noise_tmp = reverb_np(np.expand_dims(noise_tmp, axis=0), rir_np)
                wav_data = reverb_np(wav_data, rir_np)
                wav_data = wav_data + noise_tmp
            ...
            if args.cosine_similarity:
                ...
                dist = F.pairwise_distance(wav_feat.unsqueeze(-1),
                                           target_feat.unsqueeze(-1).transpose(0, 2)).detach().cpu().numpy()
                score = -1 * np.mean(dist)
            # print(score)
            if score < args.thresh:
                success += 1
                ok += 1
        print("音频：%s 攻击成功率: %2.1f" % (wav, ok / len(delay_list) * 100))

    print('%s to %s attack success rate: %2.1f' % (speaker, target, success * 1.0 / ( len(wavs) * len(delay_list) ) * 100))
    return success, len(wavs), success * 1.0 / (len(wavs) * len(delay_list)) * 100
...
# libri_speaker_file = '/data/lvyuanlong/datasets/VCTK/speaker-info.txt'
...
original_speaker = female_speaker
...
if args.target:
    if args.intra_gender:
        for i in range(len(original_speaker) // 2):
            ...
            noise1, step1, loss, noise, step2, loss1_value, loss2_value = Attack(speaker_model, enroll_speaker_feat,
                                                                                 train_speaker, speaker, target, args)
            with torch.no_grad():
            ...
            f.write(out)
            f.flush()
            wavfile.write(os.path.join(args.out_path, 'step1', speaker + '_' + target + '.wav'), 16000,
                          np.asarray(noise1, dtype=np.int16))
            wavfile.write(os.path.join(args.out_path, 'step2', speaker + '_' + target + '.wav'), 16000,
                          np.asarray(noise, dtype=np.int16))
    ...
    for i in range(len(original_speaker)//2):
        ...
        speaker = original_speaker[i]
        noise1, step1, loss, noise, step2, loss1_value, loss2_value = Attack(speaker_model, enroll_speaker_feat,
                                                                             train_speaker, speaker, speaker,
                                                                             args)
        with torch.no_grad():
            success, number, rate = Test(speaker_model, enroll_speaker_feat, test_speaker, speaker, speaker,
                                         noise, args)
        torch.cuda.empty_cache()
        out = 'Num: %02d, %5s to %5s, step1: %4d, loss: %.4f, step2: %4d, loss1: %.4f, loss2: %.4f, success number: %3d, total number: %3d, success rate: %3.2f\n' % (
            i + 1, speaker, speaker, step1, loss, step2, loss1_value, loss2_value, success, number, rate)
        f.write(out)
        f.flush()
        wavfile.write(os.path.join(args.out_path, 'step1', speaker + '_' + speaker + '.wav'), 16000,
                      np.asarray(noise1, dtype=np.int16))
        wavfile.write(os.path.join(args.out_path, 'step2', speaker + '_' + speaker + '.wav'), 16000,
                      np.asarray(noise, dtype=np.int16))
